...
{	
	...
	normal_tex = texture2D(tex1,gl_TexCoord[0].xy+offset);
	
	height = (height+normal_tex.a*depth-depth/2.0)/2.0;
	offset = height * normalize(vertex_pos).xy/* * texture2D(tex1,gl_TexCoord[0].xy+offset).z*/ * 1.0;
	offset.x *= -1.0;
		
	vec3 normalmap = texture2D(tex1,gl_TexCoord[0].xy+offset).xyz;
	vec3 normal = normalize(vec3((normalmap.x-0.5)*2.0, (normalmap.y-0.5)*2.0, normalmap.z));
	
	vec4 color_tex = texture2D(tex0,gl_TexCoord[0].xy+offset);
