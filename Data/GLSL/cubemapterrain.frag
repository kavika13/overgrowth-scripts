...
{    
    ...
    vec4 normalmap = texture2D(tex1,gl_TexCoord[1].xy);
    ...
    diffuse_color += textureCube(tex3,diffuse_map_vec).xyz;
    
    // Sample color texture and detail texture
    vec4 terrain_color = texture2D(tex4,gl_TexCoord[1].xy);
    vec4 detail = texture2D(tex0,gl_TexCoord[0].xy);
    vec4 detail2 = texture2D(tex0,gl_TexCoord[0].xy*0.25);
    ...
    float near = 0.1;
    float far = 1000.0;
    vec3 color = mix(diffuse_color, textureCube(tex3,normalize(rel_pos)).xyz, length(rel_pos)/far);
