...
{		
	// Get weights
	vec4 weight_map = texture2D(tex5,tc0);
	...
	vec4 normalmap = (texture2D(tex7 ,tc2) * weight_map[0] +
					  texture2D(tex9 ,tc2) * weight_map[1] +
					  texture2D(tex11,tc2) * weight_map[2] +
					  texture2D(tex13,tc2) * weight_map[3]);
	normalmap.xyz = UnpackTanNormal(normalmap);
	normalmap.xyz = mix(normalmap.xyz,vec3(0.0,0.0,1.0),detail_fade);
	
	vec3 ws_normal = normalize(normalMatrix * ws_from_ns * normalmap.xyz);
	...
	vec4 colormap = texture2D(tex6,tc2) * weight_map[0] +
				    texture2D(tex8,tc2) * weight_map[1] +
				    texture2D(tex10,tc2) * weight_map[2] +
				    texture2D(tex12,tc2) * weight_map[3];
...
*/
	//color = shadow_tex;//texture2D(tex10 ,tc0).xyz;
	//color = vec3(weight_map[3]*100.0);
	//color =  texture2D(tex13,tc2).xyz * weight_map[3];
	//color = normalmap.xyz;
	//color = texture2D(tex13, tc2).xyz;
